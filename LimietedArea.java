...
public class LimitedArea {
	...
	public void LoadLimitedArea() {
		try {
			...
			while(line != null) {
				...
				limitarea[index].setLongitude(this.map.get_longitude(limitarea[used].getLocation()));
				limitarea[index].setLatitude(this.map.get_latitude(limitarea[used].getLocation()));
	...
	public boolean DeleteLimitedArea() {
		...
		System.arraycopy(limitarea, deleteNumber, limitarea, deleteNumber - 1, (used - deleteNumber - 1));
		limitarea[size].setLocation(null);
		limitarea[size].setLongitude(0.0);
		limitarea[size].setLatitude(0.0);
		limitarea[size].setRadius(0.0);
		--used;
